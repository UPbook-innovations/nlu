...
class NLUPipeline(BasePipe):
    ...
    def fit(self, dataset=None, dataset_path=None, label_seperator=','):
        ...
        if hasattr(self, 'spark_estimator_pipe') is False:
        ...
        if dataset_path != None and 'ner' in self.nlu_ref:
            ...
            s_df = POS().readDataset(
                self.spark,
                path=dataset_path,
                delimiter=label_seperator,
                outputPosCol="y",
            ...
            if not self.verify_all_labels_exist(dataset):
                return nlu.NluError()
    ...
    def predict(self, data, output_level='', positions=False, keep_stranger_features=True, metadata=False,
        ...
        :param keep_stranger_features: wether to keep columns in the dataframe that are not generated by pandas.
            I.e. when you s a dataframe with 10 columns and only one of them is named text, the returned dataframe will only contain the text column when set to false
